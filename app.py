...
monthly_df = df_merged.groupby([pd.Grouper(key='day', freq='M'), 'site_id'])['SAIDI'].mean().reset_index()
fig_trend = px.line(monthly_df, x='day', y='SAIDI', color='site_id', 
                    title='SAIDI Trends: Sites Over 2023 (Rainy Season Spikes?)',
